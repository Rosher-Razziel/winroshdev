  ...
  class Cajas extends Controller{
    ...
    public function listar(){
      ...
      for ($i=0; $i < count($data); $i++) { 
        ...
        $data[$i]['ACCIONES'] =  '<div>
        <button class="btn btn-primary mb-2" type="button" onclick="btnEditarCaja('.$data[$i]['ID_CAJA'].');"><i class="fa-solid fa-pen-to-square"></i></button>
        <button class="btn btn-danger mb-2"'.$btnEliminarEstado.' type="button" onclick="btnEliminarCaja('.$data[$i]['ID_CAJA'].');"><i class="fa-solid fa-trash"></i></button>
        <button class="btn btn-success mb-2" '.$btnActivarEstado.' type="button" onclick="btnActivarCaja('.$data[$i]['ID_CAJA'].');"><i class="fa-solid fa-circle-check"></i></button>
        </div>';
    ...
    public function activar(int $id){ 
      $data = $this->model->activarCaja($id);
      echo json_encode($data, JSON_UNESCAPED_UNICODE);
      die();
    }
  }
